...
#SHOW LOGIN BOX
logindetails=$(zenity --forms --title="Thin Client Login" --text="To access the RDP Server, enter your login details." --add-entry=Username --add-password=Password --ok-label=connect --window-icon=info --separator=",")
...
case $exitstatus in
	...
	131)
		...
		xfreerdp /auth-only /v:47.226.78.191 /u:$username /p:$password
		exitstatus2=$?
